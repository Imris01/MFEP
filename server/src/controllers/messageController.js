      ...
      if (!exists) {
        tasks.push(
          createNotification({
            ...
            event: event._id,
            title,
            content: `你关注的活动「${event.title}」今天开始啦，记得查看最新进展。`,
          })
        );
      }
    }
      ...
      if (!exists) {
        tasks.push(
          createNotification({
            ...
            event: event._id,
            title,
            content: `你关注的活动「${event.title}」今天结束，别忘了确认作品与活动状态。`,
          })
        );
      }
    }
  ...
  try {
    ...
    if (message.actionType === "event_application_review") {
      ...
      await createNotification({
        ...
        content: accepted ? `你申请的活动「${event.title}」已上线` : `你申请的活动「${event.title}」未通过审核`,
      });
    ...
    if (message.actionType === "join_request") {
      const joinRequest = event.joinRequests.find((item) => item.message?.toString() === message._id.toString());
      ...
      if (accepted) {
        const exists = event.participants.some((id) => id.toString() === joinRequest.user.toString());
        if (!exists) {
      ...
      await createNotification({
        ...
        content: accepted ? `你已成功加入活动「${event.title}」` : `你提交到活动「${event.title}」的参与申请未通过`,
      });
    ...
    if (message.actionType === "event_invitation") {
      const alreadyParticipant = event.participants.some((id) => id.toString() === req.user._id.toString());
      message.status = accepted ? "accepted" : "rejected";
