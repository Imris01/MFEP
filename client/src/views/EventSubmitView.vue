...
async function submit() {
  if (!form.maidata || !form.audio || !form.cover) {
    error.value = "请先上传必需文件：maidata.txt、track.mp3 和封面图。";
    return;
  }
  ...
  <section class="panel form-panel">
    ...
    <form class="form-grid" @submit.prevent="submit">
      <label class="file-label">
        上传 maidata.txt
        <input type="file" @change="form.maidata = $event.target.files[0]" />
      </label>
      <label class="file-label">
        上传 track.mp3
        <input type="file" @change="form.audio = $event.target.files[0]" />
      </label>
      <label class="file-label">
        上传封面 bg.jpg / bg.png
        <input type="file" @change="form.cover = $event.target.files[0]" />
      </label>
      <label class="file-label">
        上传视频 pv.mp4 / bg.mp4（可选）
        <input type="file" @change="form.video = $event.target.files[0]" />
      </label>
